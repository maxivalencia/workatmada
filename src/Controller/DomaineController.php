...
class DomaineController extends AbstractController
{
     ...
     */
    public function index(DomaineRepository $domaineRepository, Request $request, PaginatorInterface $paginator): Response
    {
        $pagination = $paginator->paginate(
            $domaineRepository->findBy([], ["id" => "DESC"]), /* query NOT result */
    ...
    public function delete(Request $request, Domaine $domaine, DomaineRepository $domaineRepository): Response
    {
        if ($this->isCsrfTokenValid('delete'.$domaine->getId(), $request->request->get('_token'))) {
            $domaineRepository->remove($domaine);
        }
